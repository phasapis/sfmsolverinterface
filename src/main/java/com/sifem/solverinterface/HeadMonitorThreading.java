...
public class HeadMonitorThreading
{
        ...
        Runnable aRunnable = new Runnable()
        {
            public void run()
            {
                    ...
                    while(!endFlag.exists())
                    {
                        try {            
                            Thread.sleep(3000);
                            endFlag = new File(prop.getPropertyValue("BaseSessionFolder") + sessionid + "/" + prop.getPropertyValue("ResultsHeadPath")  + prop.getPropertyValue("EndFlag"));
                            System.out.println(" Still waiting for:" + endFlag.getAbsoluteFile());
                            
                        }
                        catch (InterruptedException ex)
                        {
                            Logger.getLogger(HeadMonitorThreading.class.getName()).log(Level.SEVERE, null, ex);
                        }
                    }
